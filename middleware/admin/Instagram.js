...
class Instagram {
  constructor({
    ...
    graphHost = process.env.GRAPH_HOST || 'https://graph.facebook.com',
    graphVersion = 'v23.0',
    requestTimeoutMs = 30_000,
  } = {}) {
    this.accessToken = accessToken;
    this.igUserId = igUserId;
    this.graphBase = `${graphHost}/${graphVersion}`;
  ...
  async createReelContainer({ description, videoUrl }) {
    ...
    try {
      const { data } = await this.http.post(
        `${this.graphBase}/${this.igUserId}/media`,
        null,
        {
          params: {
            caption: description,
      ...
      const msg = error?.response?.data?.error?.message || error.message || 'Failed to create IG container';
